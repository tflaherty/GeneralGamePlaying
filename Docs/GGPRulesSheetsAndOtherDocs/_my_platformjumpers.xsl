<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:choose>   
    <xsl:when test="//structure[name='cell' and arg[1]='red' and arg[2] = $row and arg[3] = $col]"> 
      <xsl:attribute   name="style">background-color: #FFCCCC</xsl:attribute> 
    </xsl:when>   
    <xsl:when test="//structure[name='cell' and arg[1]='blue' and arg[2] = $row and arg[3] = $col]"> 
      <xsl:attribute   name="style">background-color: #99CCCC</xsl:attribute> 
    </xsl:when>   
  </xsl:choose>
  <center>
  <xsl:choose>   
    <xsl:when test="//structure[name='jumper' and   arg[2]=$row and arg[3]=$col and arg[1]='red']">  
      <img class="piece"   src="/gamemaster/games/platformJumpers/red.png"/> 
    </xsl:when>  
    <xsl:when test="//structure[name='jumper' and arg[2]=$row and   arg[3]=$col and arg[1]='blue']"> 
      <img class="piece"   src="/gamemaster/games/platformJumpers/blue.png"/> 
    </xsl:when> 
  </xsl:choose>
  </center>  
  </td>  

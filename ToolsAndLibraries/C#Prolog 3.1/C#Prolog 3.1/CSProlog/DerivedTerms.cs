...
namespace Prolog
{
  public partial class Engine
  {
    ...
    public class SqlTerm : BaseTerm
    {
      ...
      public bool IsOpen { get { return (dr != null); } }
      static List<SqlTerm> possiblyOpenQueries;

      static SqlTerm ()
      {
        possiblyOpenQueries = new List<SqlTerm> ();
      }

      public SqlTerm (string command, bool showColNames)
      {
        functor = this.command = command;
      ...
      public void ExecuteQuery (DbProviderFactory pf, DbConnection cn)
      {
        DbCommand cmd = null;
        ...
        catch (Exception x)
        {
          ...
          IO.Fatal ("Error while attempting to set up SQL command.\r\n" +
                    "Command: {0}\r\nConnectstring: {1}\r\nMessage: {2}",
                    command, cmd.Connection.ConnectionString, x.Message);
        }
      ...
      public int ExecuteNonQuery (DbProviderFactory pf, DbConnection cn)
      {
        DbCommand cmd = null;
        int rowsAffected = 0;
        ...
        List<Type> numericTypes = new List<Type>
      {
        ...
        if (dr.Read () || (dr.NextResult () && dr.Read ())) // handle multiple result sets
        {
          for (int i = dr.FieldCount - 1; i >= 0; i--)
          {
            ...
            if (type == typeof (string))
            {
              ...
              // OLD, can't remeber why I did this
              //if (showColNames)
              //  t = new AtomTerm (s.Enquote ('\''));
              //else if (string.IsNullOrEmpty (s) || s.Length <= 64) // arbitrary choice
              //  t = new AtomTerm (s.Dequoted ());
              //else
              //  t = new StringTerm (s);
            ...
            //IO.Fatal ("SqlNextRecordToListTerm: Unmapped type '{0}'", type.Name);
      ...
      public static void PostQueryTidyUp () // called at end of (Prolog) command execution
      { // use of 'foreach' gives exception ("modified collection")
        for (int i = 0; i < possiblyOpenQueries.Count; i++) possiblyOpenQueries [i].Close ();
      }
    }
    #endregion SqlTerm
    ...
    public class FileReaderTerm : FileTerm
    {
      ...
      public FileReaderTerm (Engine engine, string fileName)
      {
        this.engine = engine;
      ...
      public FileReaderTerm (Engine engine, TextReader tr)
      {
        this.engine = engine;
      ...
      public void Open ()
      {
        ...
        {
          engine.Throw (Engine.IOException, 
    ...
    public class FileWriterTerm : FileTerm
    {
      ...
      public FileWriterTerm (Engine engine, string fileName)
      {
        this.engine = engine;
        functor = this.fileName = fileName;

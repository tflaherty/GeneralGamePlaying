...
*/
       ...
       X ; Y  :== or. % just for preventing ';' from redefinition by the user
...
%         (A == B ; A \= B), !. % SWI-Prolog
    ...
    */
       ...
       sql_select( S, X) :-      % SELECT-statement(s) in string S, multiple result sets allowed
         sql$select( State, S, false), % true/false only to distinguish between select and select2
         !,
         sql$select( State, S, X).

       sql$select( State, S, X) :== sql_select. % next record from the SqlReader, fails at end
       sql$select( State, S, X) :-
         !,
       ...
       sql_select2( S, X) :-
         sql$select( State, S, true), % show column names
         !,
         sql$select( State, S, X).
       ...
       % sql_connect retrieves the value of the provide and the connection string from
       % the config file. C is the key under which these values are stored.
       sql_connect( C, A)    :== sql_connect. % see 'help( sql_connect).'
       sql_command( C)       :== sql_command. % arbitrary SQL-command(s) in string C, ...
       sql_command( C, N)    :== sql_command. % ... output N is the number of rows affected

       % The two predicates below can be used as an alternative
       % to sql_connect/2, and also to retrieve the current values.

       sql_provider( P)      :== sql_provider.
       sql_connectstring( C) :== sql_connectstring.
       ...
       */
       ...
       xml_term( [_|_], X, T)   :== xml_term. % controls (items in a list): see below
       ...
       json_format( J0, J1)     :== json_format. % J1 is the formatted version of J0

       /* XMLPL 'CONTROLS' TO BE IMPLEMENTED:
        * For this release, the controls applying to inbound (xml -> term)
        * parsing are:
        *
...
%      pattern matching
       ...
       patmat( P, T) :-         % returns pattern P that matches T
         patmat$( State, P, T),
         !,
         patmat$( State, P, T).

       patmat$( State, P, T) :== patmat.
       patmat$( State, P, T) :-
         !,
         nonvar( State),        % 'clause' resets State to var upon failure
         patmat$( State, P, T).

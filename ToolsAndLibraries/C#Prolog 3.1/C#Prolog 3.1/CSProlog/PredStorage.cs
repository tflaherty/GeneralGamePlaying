...
namespace Prolog
{
  public partial class Engine
  {
    ...
    public class PredicateTable
    {
      ...
      public PredicateTable (Engine engine)
      {
        this.engine = engine;
      ...
      public void HandleSimpleDirective (string directive, string argument, int arity)
      {
      ...
      public void AddClause (ClauseNode clause)
      {
        ...
        else // first predicate or different predicate
        {
          ...
          {    // If from same, IsDiscontiguous must hold, unless DiscontiguousAllowed = "1" in .config
            ...
            {
              if (b)
                IO.Warning ("Predicate '{0}' occurs discontiguously but is not declared as such", index);
...
//          IO.Write (" {0}{1}", (pd.IsFirstArgMarked (clause))?".":" ", nextClause.Term);
      ...
      public bool ListAll(string functor, int arity, bool showPredefined, bool showUserDefined, out string outputString)
      {
          bool result = false; // no predicate <functor>/<arity> assumed
          PredicateDescr pd;
          var sb = new StringBuilder();
          ...
          string clauseOutputString;
          foreach (KeyValuePair<string, PredicateDescr> kv in sl)
          {
              result = ListClause(pd = kv.Value, pd.Functor, pd.Arity, ++seqNo, out clauseOutputString) || result;
              sb.AppendLine(clauseOutputString);
          }
      ...
      void SetupCrossRefTable () //TODO: deal with arguments of not/1 and call/1
      {
      ...
      public bool ShowHelp (string functor, int arity, out string suggestion)
      {
        ...
        if (functor == null)
        {
          ...
          IO.WriteLine ("\r\n  Consult CsPrologHelp.txt for help on re-creating help.");
        ...
        const string DASHES = 
          "|------------------------------------------------------------------------------------------";
        IO.WriteLine ("{0}{1}{2}{1}{0}", DASHES, Environment.NewLine, sb.ToString ());

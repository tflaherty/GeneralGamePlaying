...
namespace Prolog
{
  public enum BI // builtins
  {
    ...
    inc_counter, integer, is_, json_xml, json_format, le_num, le_ord, leapyear, length, list, listing, 
    listing0, listing0X, listing0XN, listingX, listingXN, lt_num, lt_ord, make_array, make_help_resx, 
    maxwritedepth, member, name, ne_num, ne_str, ne_uni, nl, nocache, nodebug, nonvar, noprofile, nospy, 
    nospyall, notrace, noverbose, now, number, numbervars, numcols, or, patmat, pp_defines, predicatePN, 
    predicateX, print, profile, put, query_timeout, read, readatoms, readatom, readeof, readln, regex_match, 
    regex_replace, retract, retractall, reverse, sendmail, see, seeing, seen, set_counter, setenvvar, setvar, 
    shell_d, shell_p, shell_x, shell_sync_d, shell_sync_p, shell_sync_x, shell_dos, shell_exe, showfile, 
    showprofile, silent, sort, spy, spypoints, sql_connect, sql_connectstring, sql_provider, sql_command, 
    sql_select, callstack, statistics, string_, string_datetime, string_term, string_words, stringstyle, 
    ...
    workingdir, write, writef, writeln, writelnf, xml_term, xmltrace
  }
      ...
      switch (biId)
      {
        ...
        case BI.writelnf: // writef( X, L) // formatted writeln, L last
          if (!(term.Arg (0) is StringTerm))
            IO.Error ("First argument of writelnf/2 must be a string");

          if (!(term.Arg (1) is ListTerm))
            IO.Error ("Second argument of writelnf/2 must be a list");
        ...
        case BI.console:
          ...
          if (term.Arity == 2)
          {
            ...
            IO.WriteLine (term.Arg (0).FunctorToString);
        ...
        #region SQL
        case BI.sql_connect: // sql_connect( Provider, Args)
          ...
          if (t1.IsAtom || t1.IsString)
            ...
            return false;
          DbConnectionInfoFromConfig (t0.FunctorToString, t1);
          break;

        case BI.sql_connectstring: // sql_connectstring( ?C) - set DB connectstring to C
          if ((t0 = term.Arg (0)).IsVar)
            term.Arg (0).Unify (new StringTerm (sqlConnectstring == null ? "(not set)" : sqlConnectstring), varStack);
          else if (t0.IsAtom || t0.IsString)
            sqlConnectstring = t0.FunctorToString;
          else
            return false;
          break;

        case BI.sql_provider: // sql_provider( ?P) - set DB provider to P
          if ((t0 = term.Arg (0)).IsVar)
            term.Arg (0).Unify (new StringTerm (sqlProvider == null ? "(not set)" : sqlProvider), varStack);
          else if (t0.IsAtom || t0.IsString)
            sqlProvider = t0.FunctorToString;
          else
            return false;
          break;

        // sql_command( C) - execute DB command C (insert, delete, update, ...)
        case BI.sql_command: // sql_command( C[, N]) -- Execute SQL-command C. Number of rows afected is N
          t0 = term.Arg (0);
          string command;
          int rowsAffected;
          ...
          if (t0 is ListTerm)
          {
            if ((command = Utils.Format ((ListTerm)t0)) == null) return false;
          }
          else
            command = t0.ToString ().Dequoted ();

          sqlt = new SqlTerm (command, false);
          DbOpenConnection (); // creates dbProviderFactory and dbConnection
          rowsAffected = sqlt.ExecuteNonQuery (dbProviderFactory, dbConnection);

          if (term.Arity == 2 && !term.Arg (1).Unify (new DecimalTerm (rowsAffected), varStack))
            return false;

          break;

        case BI.sql_select: // sql_select( State, S, X) - get the next X from State
          t0 = term.Arg (0);
          ...
          if (t0.IsVar) // first call only
          {
            ...
            if (t1 is ListTerm)
            {
              if ((query = Utils.Format ((ListTerm)t1)) == null) return false;
            }
            else
              query = t1.ToString ().Dequoted ();

            sqlt = new SqlTerm (query, (term.Arg (2).HasFunctor ("true"))); // cf. select2/2
            DbOpenConnection (); // creates dbProviderFactory and dbConnection
            sqlt.ExecuteQuery (dbProviderFactory, dbConnection);
            term.Arg (0).Unify (sqlt, varStack); // bind the SqlTerm to T
          ...
          while (true)
          {
            ...
            if (term.Arg (2).Unify (lt, varStack)) break;
          }
        ...
        case BI.atom_string: // atom_string( ?A, ?S)
          ...
          if (t1.IsVar) // create a last containing A's characters or character codes
          {
            t0 = term.Arg (0);
        ...
        case BI.shell_exe: // shell_exe( X [,Args] [,wait ,E]). 
          ...
          proc.StartInfo.Arguments = ConvertToCmdArgs (term.Arg (1));
          proc.Start ();
        ...
        //case BI.json_xml: // json_xml( +J, ?X, ) converts JSON term to XML string or file
        //  string rootTagName;
        //  string [] attributes = null; // will appear as attribute rather than <value>...</value>

        //  if (term.Arity >= 3)
        //  {
        //    t0 = term.Arg (0); // json
        //    t1 = term.Arg (1); // xml structure root tag name
            
        //    if (!(t1.IsString || t1.IsAtom))
        //      IO.Error ("json_xml/3/4: second argument (root tag name) must be a string or atom");

        //    rootTagName = t1.FunctorToString;
        //    t2 = term.Arg (2); // xml

        //    if (term.Arity == 4)
        //    {
        //      t3 = term.Arg (3); // list with attribute names

        //      if (!t3.IsProperList)
        //        IO.Error ("json_xml/4: fourth argument (root tag name) must be a list");

        //      attributes = ((ListTerm)t3).ToStringArray ();
        //    }
        //  }
        //  else
        //  {
        ...
        //  else
        //  {
        //    x = t2.FunctorToString;
        //    outFile = (t2.Arity == 1 && x == "tell"); // ... or the functor of a destination file containing the XML structure?

        //    if (outFile)
        //    {
        //      x = Utils.FileNameFromTerm (t2.Arg (0), ".json");

        //      if (x == null) return false;
        //    }
        //  }

        //  JsonToXml (t0, rootTagName, ref x, attributes);

        //  if (!outFile && !t2.Unify (new StringTerm (x), varStack)) return false;

        //  break;

        case BI.json_format: // json_format( +J0, -J1) % J1 is the formatted version of J0
          if ((t0 = term.Arg (0)).IsVar || !(t1 = term.Arg (1)).IsVar) return false;

          if (!t1.Unify (new StringTerm (FormatJson (t0)), varStack)) return false;
          break;
        ...
        case BI.regex_match: // regex_match( +Source, +Pattern, ?Result [, Options])
          ...
          string [] options = (term.Arity == 4) ? ((ListTerm)term.Arg (3)).ToStringArray () : null;
          BaseTerm groups = Utils.FindRegexMatches (opTable, t0.FunctorToString, t1.FunctorToString, options);
        ...
        case BI.get_counter:
          globalTermsTable.getctr (term.Arg (0).To<int> (), out cntrValue);

          if (!term.Arg (1).Unify (new DecimalTerm (cntrValue), varStack)) return false;
          break;

        case BI.set_counter:
          globalTermsTable.setctr (term.Arg (0).To<int> (), term.Arg (1).To<int> ());
          break;

        case BI.inc_counter:
          n = term.Arg (0).To<int> ();
          globalTermsTable.getctr (n, out cntrValue);

          if (term.Arity == 2 &&
              !term.Arg (1).Unify (new DecimalTerm (cntrValue+1), varStack)) 
            return false;
          
          globalTermsTable.incctr (n);
          break;

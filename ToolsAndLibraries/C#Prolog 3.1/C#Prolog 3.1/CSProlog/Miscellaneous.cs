...
namespace Prolog
{
  public partial class Engine
  {
    ...
    public static bool ProcessArgs (Engine e, string [] args, bool windowsMode)
    {
      ...
      e.Query = command + (command.EndsWith (".") ? null : "."); // append a dot if necessary
      ...
      if (msg == null)
      {
        ...
        foreach (Engine.ISolution s in e.SolutionIterator)
        {
          if (e.Error || (!found && !s.Solved)) // only an immediate 'no' give rise to an error
          {
            msg = s.ToString ();
    ...
    public static class Utils
    {
      ...
      public static bool CreateHelpResourceFile (out string resxFileName)
      {
        ...
        if (Engine.ConfigSettings.CsPrologHelpFile == null) // set in CSProlog.exe.config
          Console.WriteLine ("No help file name found in config file (key 'CsPrologHelpFile')");
        else
        {
          string helpFileName = Engine.ConfigSettings.CsPrologHelpFile;
          int lineNo = 0;
          ...
          try
          {
            while (!sr.EndOfStream)
            {
              ...
              if (m.Success)
              {
                ...
                {
                  // finish off previous content
                  rw.AddResource (key, content.ToString ().TrimEnd ());
                  content.Length = 0; // clear content
                }
                ...
                combinedArities [functor] =
                  ...
                  (arity == "*" ? "(*)" : '/' + arity);
                maxPredLen = Math.Max (maxPredLen,
                                       functor.Length + (arities == null ? 0 : arities.Length) + arity.Length + 1);
              }
  ...
  public static class Extensions
  {
    ...
    public static string RemoveUnnecessaryAtomQuotes (this string s)
    {
      ...
      // check whether the a is quoted at all
    ...
    public static string Unescaped (this string s)
    {
      ...
      Engine.IO.Error ("Unrecognized escape sequence in string \"{0}\"" +
